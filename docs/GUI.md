...
창은 위에서부터 **입출력 파일 → 스템 모델 → 마이너스원 → 카운트인 → 드럼 분리 → 기타** 순의
대분류로 정리돼 있습니다:

1. **입출력 파일**: 음원 파일을 **드래그 앤 드롭**(또는 "파일 선택"). 출력 폴더는
   입력 파일 옆 `BandPrepareOutput/<곡이름>` 으로 자동 설정됩니다(직접 바꿔도 됨).
   **출력 포맷**(wav/mp3/flac)도 여기서 고릅니다.
   - **유튜브 링크 / YouTube link**: 파일 대신 **유튜브(또는 다른 사이트) 링크**를
     붙여넣으면 음원을 자동으로 받아 분리합니다(링크를 넣으면 파일 선택은 무시됨).
     받은 원본은 결과 폴더에 `source.<확장자>` 로 함께 저장됩니다. 출력 폴더를 비워
     두면 **홈 폴더의 `BandPrepareOutput/<영상 제목>`** 에 만들어집니다. 진행 막대는
     **다운로드 → 분리** 순으로 한 번에 이어서 움직입니다.
...
4. **카운트인**(선택): 마이너스원 **맨 앞에 클릭(삐 삐 삐 삐)** 을 붙여 박자를 맞춰
   들어갈 수 있게 합니다. **템포(BPM)** 는 곡에 맞춰 직접 입력하고, **마디 수**·**박자/마디**로
   길이를 조절합니다(기본 1마디·4박). 마이너스원을 켰을 때만 쓸 수 있습니다(꺼져 있으면 비활성화).
5. **드럼 분리**(선택, 기본 꺼짐): 제목 체크박스를 켜면 **드럼 모델**(기본 `mdx23c`)을
   고를 수 있고 드럼 스템을 킥/스네어 등 조각으로 나눕니다. 켜는 순간
   **드럼 스템 유지**가 자동으로 함께 켜집니다(원하면 해제 가능).
6. **분리 시작** → 진행 막대 + 로그로 진행 확인. 모델이 도는 동안에도 진행 막대가
