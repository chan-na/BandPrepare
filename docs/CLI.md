...
| **인터넷 연결** | 첫 실행 때 모델 가중치를 내려받습니다(이후엔 캐시되어 오프라인 가능) |
...
> MDX23C 약 438MB). 한 번 받으면 캐시되어 다음부터는 빠릅니다(번들 바깥
...
(그러면 `instruments/drums.wav` 도 생김) 원하는 트랙만 섞으세요.
...
> 드럼 세부 분리를 한 상태에서도 원본 `drums.wav` 는 기본으로 함께 저장되므로 그대로
> 합치면 됩니다(저장하지 않으려면 `--no-keep-drums-stem`).
...
```bash
bandprepare-cli 내곡.mp3 --stems drums
```
...
# 드럼 세부 분리 모델 바꾸기
bandprepare-cli 내곡.mp3 --drum-model drumsep              # 4조각(kick/snare/toms/cymbals), MIT
bandprepare-cli 내곡.mp3 --drum-model larsnet              # 5조각(+hihat), 비상업 라이선스
bandprepare-cli 내곡.mp3 --drum-model mdx23c               # 6조각(+ride/crash 분리), 기본값, MIT
```
...
```bash
# 드럼 세부 분리(2단계) 생략 → 악기 트랙만, 훨씬 빠름
bandprepare-cli 내곡.mp3 --no-drum-split

# 드럼 Wiener 후처리 끄기 → 드럼 단계 가속(품질 약간 손해, larsnet 전용)
bandprepare-cli 내곡.mp3 --drum-model larsnet --no-drum-wiener
```
