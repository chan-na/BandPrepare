...
> LarsNet 약 562MB). 한 번 받으면 캐시되어 다음부터는 빠릅니다(번들 바깥
...
```bash
# 악기 분리 모델 바꾸기
bandprepare-cli 내곡.mp3 --stem-model htdemucs_ft          # 4스템(보컬/드럼/베이스/other), 고품질
...
bandprepare-cli 내곡.mp3 --drum-model larsnet              # 5조각(+hihat), 기본값(비상업 라이선스)
bandprepare-cli 내곡.mp3 --drum-model mdx23c               # 6조각(+ride/crash 분리), MIT
```
...
bandprepare-cli 내곡.mp3 --no-drum-wiener
```
