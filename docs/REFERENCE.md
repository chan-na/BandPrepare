...
| `yt-dlp` | `>=2024.0` | 유튜브/URL 입력 음원 다운로드(순수 Python) |

> ⚠️ **번들 yt-dlp 노후화**: yt-dlp는 사이트(특히 유튜브) 변경에 맞춰 자주 갱신됩니다.
> 포터블 앱에 들어간 yt-dlp는 시간이 지나면 막힐 수 있으니, 다운로드가 실패하면 **최신
> 릴리스로 갱신**하세요. 소스/pip 설치 시에는 `pip install -U yt-dlp` 로 개별 갱신이 가능합니다.
...
```
bandprepare-cli <input_audio | URL> [options]
  # <input_audio>: mp3/wav/flac/m4a 등 로컬 파일
  # <URL>: 유튜브 등 링크 → yt-dlp 로 음원을 받아 결과 폴더에 source.<ext> 로 저장 후 처리
  #        (-o 미지정 시 현재 폴더의 BandPrepareOutput/<영상 제목>/)
  ...
  --count-in BPM                마이너스원 앞에 해당 BPM 의 클릭(메트로놈) 카운트인 추가 (--minus 필요)
                                → 파일명에 -count<BPM> 표시 (예: minus-bass-count120.wav)
  --count-in-bars N             카운트인 마디 수 (기본: 1)
  --count-in-beats N            한 마디 박자 수 (기본: 4 = 4/4박자)
