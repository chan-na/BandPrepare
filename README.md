...
> 🖱 **터미널이 어렵다면?** 마우스로 쓰는 **GUI**도 있습니다 → 아래
> ["마우스로 쓰기 (GUI)"](#-마우스로-쓰기-gui) 참고.
...
누르면 됩니다(CLI와 **같은 코어**를 쓰므로 결과물·모델·옵션은 동일).
...
> 더블클릭으로 실행하는 **포터블 앱**(ffmpeg/Python/torch 사전 설치 불필요)으로 만들려면
> 아래 ["포터블 앱 빌드"](#포터블-앱-빌드-pyinstaller)를 참고하세요.
...
>
> base 의존성에는 정적 ffmpeg를 동봉하는 `imageio-ffmpeg (>=0.5)` 도 포함됩니다 —
> 시스템 ffmpeg가 없을 때(특히 포터블 번들) 압축 음원 디코딩에 사용됩니다.
